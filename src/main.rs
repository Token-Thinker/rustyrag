...
async fn axum(
    ...
    let embedding = env::var("EMBEDDING")
    ...
    let project_dir_str = env::var("PROJECT_DIR").unwrap_or_else(|_| ".".to_string());
    let project_dir = PathBuf::from(project_dir_str);
    let endings = &["rs", "md", "toml"];
    let files = contents::load_files_from_dir(project_dir.clone(), endings, &project_dir)?;
